...
> **適用於 Windows 測試人員組建17093和更新版本**

會在每次使用`wsl.conf`啟動子系統時, 自動在 WSL 中設定特定功能。 

目前, 這包括自動掛載選項和網路設定。

`wsl.conf`位於中`/etc/wsl.conf`的每個 Linux 發行套件中。 如果檔案不存在, 您可以自行建立。 WSL 會偵測檔案是否存在, 並將讀取其內容。 如果檔案遺失或格式不正確 (也就是不正確的標記格式), WSL 會繼續正常啟動。

以下是您可以`wsl.conf`新增至發行版本的範例檔案:
