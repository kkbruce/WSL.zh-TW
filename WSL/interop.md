...
> 適用於 Windows 測試人員組建 17063 與更新版本。
...
不支援使用 Windows 應用程式修改位於 VolFs `/mnt/<x>`(不在下) 的檔案。  根據預設, WSL 會嘗試將 Windows 二進位檔的工作目錄維持為目前的 WSL 目錄，但是如果工作目錄是在 VolF 上，則會在執行個體建立目錄上回復。
